...
| # | 内容 | 備考 |
|---|---|---|
| 1 | **月データ削除機能** | 月間・週間タブに「対象月のデータを削除」ボタンを追加 |
| 2 | **削除前アーカイブ確認** | 削除ボタン押下時に「CSVへアーカイブ保存しますか？」確認ダイアログを表示。YESの場合はCSV出力してから削除 |
| 3 | **ユーザー情報編集** | ユーザー名・パスワード・メールアドレスの変更画面（設定タブまたは専用画面） |
| 4 | **PDF出力** | A4固定2枚（1枚目:明細、2枚目:グラフ＋合算値） |
| 5 | **LRUキャッシュ** | 6ヶ月・TTL 当月120分／過去月24時間（RULES §2.2） |
