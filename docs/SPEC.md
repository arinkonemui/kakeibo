...
### 8.4 アーカイブCSV読み込み表示
- 出力タブに「アーカイブCSVを読み込む」
- **取り込み月にレコードが既に存在する場合は取り込み不可**（エラーメッセージを表示）
- 読み込み後は月間・週間・集計タブでアーカイブデータを参照可能
- **過去月の場合（当月以外）：** アーカイブ表示モード（読み取り専用）
  保存不可（文言：**「アーカイブ表示のため保存できません」**）
- **当月の場合：** 取り込んだエントリを未保存変更として ops キューに追加し、通常の入力・保存フローで使用可能
  カテゴリ名で実カテゴリ ID に自動マッピング。一致しないカテゴリはスキップ（件数を通知）
- 復元ボタンは作らない
