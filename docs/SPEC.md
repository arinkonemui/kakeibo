...
## 0. ブランディング / SEO / トーン
### タイトル（ページタイトル）
おさいふノート｜家計簿を手帳のように見開き管理。シンプルなWeb家計簿

### メタディスクリプション（案）
おさいふノートは、家計簿を手帳感覚で見開き管理できるWebアプリです。1か月分の収支をカレンダー形式でパッと俯瞰できるから、お金の流れが一目でわかります。余計な機能を削ぎ落としたシンプル設計で、これまで挫折した方でも続けやすいのが特徴です。
...
### カラーパレット方針
- 暖色系（左側サンプル）で統一
- 参考（候補）: #FF9F43 / #FF6B6B / #F7C59F
- ペーパーカラー（原案）: #FFTAF（実装時は6桁HEXに正規化して扱う）
...
### 8.4 アーカイブCSV読み込み表示
- 出力タブに「アーカイブCSVを読み込む」
- CSV読み込み時は存在チェックを行わず、即座にアーカイブ表示へ移行する
- 読み込み後は月間・週間・集計タブでアーカイブデータを参照可能（読み取り専用）
  保存不可（文言：**「アーカイブ表示のため保存できません」**）
- バナーに **「DBへ反映」** ボタンを表示し、押下時に存在チェックを行う
  - **取り込み月にレコードが既に存在する場合：** エラーメッセージを表示し反映不可
  - **取り込み月にレコードが存在しない場合：** カテゴリ名で実カテゴリ ID に自動マッピングし直接 DB へ保存
    一致しないカテゴリはスキップ（件数を通知）。保存後は対象月の通常表示へ遷移
- 復元ボタンは作らない

## 9. マルチユーザー・データ管理（B案）
### 9.1 ユーザー分離
- user_id により完全分離（他ユーザーのデータが混ざらない）
- user_id は認証から導出される（クエリパラメータ等でクライアントが指定するものではない）

### 9.1.1 user_id 正規形式
- 形式: `u_` + 32文字の小文字16進数（SHA-256ハッシュの先頭16バイト）
- 合計34文字固定（例: `u_a1b2c3d4e5f60718293a4b5c6d7e8f90`）
- 導出ルール: 認証で得られる識別子（メールアドレス等）を SHA-256 でハッシュし、先頭16バイトを hex にして `u_` を付与
- 生のメールアドレスや個人情報を user_id に含めない（プライバシー保護）
- ログやURLに安全に含められる形式とする
...
- 「直近6ヶ月」の判定は **月キー（YYYY-MM）単位** とする（日付差分で「6ヶ月前日」などの計算はしない）
...
- 編集可能範囲：
  - 直近6か月（当月 + 過去5か月）まで編集可能
  - 判定は **月キー（YYYY-MM）単位** とする（日付差分での境界判定はしない）
  - それより古い月は編集不可（閲覧・出力・アーカイブCSV表示のみ）
- 競合チェック：
  - 保存時は **編集対象月** の months.version の一致を必須
...
- 詳細は docs/DB_SCHEMA.md および db/schema.sql（ソースオブトゥルース） を参照